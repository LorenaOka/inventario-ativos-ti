...
const validarResposta = async (question) => {
  while (true) {
    const answer = (await pergunta(question)).trim().toLowerCase();
...
const cadastrarFuncionario = async () => {
  try {
    const cpf = (await pergunta("CPF (formato: xxx.xxx.xxx-xx): ")).trim();
...
const excluirFuncionario = async () => {
  try {
    const cpf = (
      await pergunta(
        "CPF do funcionário a ser excluído (formato: xxx.xxx.xxx-xx): "
      )
    ).trim();
...
const consultarInventarioFuncionario = async () => {
  try {
    const cpf = (
      await pergunta("CPF do funcionário (formato: xxx.xxx.xxx-xx): ")
    ).trim();
    const funcionario = await obterFuncionarioPorCPF(cpf);
